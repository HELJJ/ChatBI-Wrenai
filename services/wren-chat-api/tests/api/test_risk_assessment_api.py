...
_SAMPLE_EXTRACTION = RiskAssessmentStats(
    filename="报告.docx",
    riskHigh=1,
    riskHighRate=0.02,
    riskMedium=5,
...
class FakeRiskAssessmentService:
    def __init__(
        ...
        result: RiskAssessmentStats | None = None,
    ):
        self.error = error
        self.result = result
...
async def test_extraction_success_returns_structured_result(tmp_path):
    ...
    assert body["data"]["filename"] == "报告.docx"
    assert body["data"]["riskHigh"] == 1
    assert body["data"]["riskHighRate"] == 0.02
    assert body["data"]["riskMedium"] == 5
    assert body["data"]["riskMediumRate"] == 0.09
    assert body["data"]["riskLow"] == 46
    assert body["data"]["riskLowRate"] == 0.87
    assert body["data"]["finalEvaluationCode"] == "L"
    assert body["data"]["finalEvaluationName"] == "低风险"
...
async def test_non_doc_suffix_answers_business_code_422(tmp_path):
    app, _ = make_app(tmp_path)
    async with client_for(app) as client:
        response = await client.post(
            "/v1/risk-assessment/extract",
    ...
    assert body["message"] == InvalidRiskFile().public_message


async def test_bytes_without_zip_or_ole_magic_answer_business_code_422(tmp_path):
    app, _ = make_app(tmp_path)
    async with client_for(app) as client:
        response = await client.post(
            "/v1/risk-assessment/extract",
...
@pytest.mark.parametrize(
    ...
    [
        (InvalidRiskFile(), 422),
        (RiskAssessmentDataNotFound(), 422),
        (RiskDocConversionFailed(), 502),
        (RiskAssessmentOutputMalformed(), 502),
        (
            InvalidRiskAssessmentResult(),
            502,
        ),
...
async def test_service_errors_answer_http_200_business_codes(
