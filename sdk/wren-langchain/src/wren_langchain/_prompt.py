...
``toolkit.get_tools(include_memory_write=...)``. If a caller hides
...
def build_system_prompt(toolkit: WrenToolkit, *, tools: Iterable | None = None) -> str:
    """Render the system prompt.

    ``tools`` is the actual tool list used by the agent. When ``None``, the
    toolkit's default ``get_tools()`` output is used. Pass the same list you
    pass to ``create_agent(..., tools=...)`` so the prompt stays in sync —
    e.g., when ``include_memory_write=False`` is used to drop
